...
Кнопки над `scripts/wwan-up.sh`: **Проверка** (`--check`) / **Включить** (`--system` —
полный подъём модема/PPP и раздача интернета приложениям Android одной кнопкой) /
**Выключить** (`--down`) / **Интернетометр** (открыть `https://internet.yandex.ru` через
системный `ACTION_VIEW` — визуально проверить, что интернет действительно есть). Ничего не
выполняется само — ни на `BOOT_COMPLETED`, ни при открытии приложения, только по нажатию.
...
приложение решение об установке не принимает — собранный `F515WwanApp.apk` просто лежит
рядом.
