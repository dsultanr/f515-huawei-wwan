...
static const struct usb_device_id option_ids[] = {
	...
	{ USB_DEVICE_AND_INTERFACE_INFO(HUAWEI_VENDOR_ID, HUAWEI_PRODUCT_E1750, 0xff, 0xff, 0xff),
	  .driver_info = RSVD(2) },
	/* MERGE: our own modem, 12d1:1506, storage-mode-switched Huawei stick,
	 * vendor-specific class=0xff/sub=0x02 interfaces 0-2, not covered by
	 * any existing entry above (all require an exact PID match or
	 * sub=0x01) - match on bare VID:PID, let calc_num_ports skip any
	 * interface with no bulk endpoints. */
...
static struct usb_serial_driver * const option_serial_drivers[] = {
	&option_1port_device, NULL
};

/* MERGE: was module_usb_serial_driver(serial_drivers, option_ids) - now
 * called explicitly from usb_serial_init() in usb-serial.c so everything
 * lives in one .ko with no cross-module EXPORT_SYMBOL needed. */
int option_module_init(void)
{
	return usb_serial_register_drivers(option_serial_drivers,
					   "option", option_ids);
}

void option_module_exit(void)
{
	usb_serial_deregister_drivers(option_serial_drivers);
}
