...
/* MERGE: option.c's registration, now compiled into this same module */
int option_module_init(void);
void option_module_exit(void);
...
static int __init usb_serial_init(void)
{
	...
	pr_err("MERGEDIAG: usb_serial_init entered\n");

	usb_serial_tty_driver = alloc_tty_driver(USB_SERIAL_TTY_MINORS);
	if (!usb_serial_tty_driver)
		return -ENOMEM;

	pr_err("MERGEDIAG: alloc_tty_driver ok\n");

	/* Initialize our global data */
	result = bus_register(&usb_serial_bus_type);
	pr_err("MERGEDIAG: bus_register result=%d\n", result);
	if (result) {
	...
	/* MERGE: option.c registration, called directly instead of via its
	 * own module_init since everything is one .ko now */
	result = option_module_init();
	if (result < 0) {
		pr_err("%s - registering option driver failed\n", __func__);
		goto exit_generic;
	}
